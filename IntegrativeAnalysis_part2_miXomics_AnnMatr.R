...
resultsDir <- file.path(workingDir, "results/mixomics.ann") 
...
## ----loadPreviousData----------------------------------------------------
# Load files
load(file=file.path(dataDir, "annot.data.Rda"), verbose = TRUE)
GenesSelected <- gene.expand.matrix # NO cal transposar pq ja venen en format mostres+categs (files) x feats (cols)
ProtsSelected <- prot.expand.matrix # NO cal transposar pq ja venen en format mostres+categs (files) x feats (cols)
dim(GenesSelected)
dim(ProtsSelected)
...
grid1 <- seq(0.1, 2, length = 12)
grid2 <- seq(0.001, 0.02, length = 10)
###################################################################
###################################################################
###################################################################
###################################################################
...
###################################################################
###################################################################
#save(cv.score, file=file.path(resultsDir, "cv.score.Ann.Rdata"))
###################################################################
###################################################################
###################################################################
###################################################################
...
#if(!(exists("cv.score"))) load(file=file.path(resultsDir, "cv.score.Rdata"), verbose = TRUE)
if(!(exists("cv.score"))) load(file=file.path(resultsDir, "cv.score.Ann.Rdata"), verbose = TRUE)
...
col.groups
#c(col.groups, rep(4,28))##FFF 28 is num categs!!!
...
## ----corrCirclePlot06, out.width='.8\\\\linewidth'-----------------------
...
## ----corrCirclePlot07, out.width='.8\\\\linewidth'-----------------------
